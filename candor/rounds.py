"""Round-split engine — splits an overall admit chance across ED/EA/RD rounds.

Extracted verbatim from app.py (pure move, no behavior change). Calibrated
against 2,350 real stored round-odds from 384 users. Everything here is pure:
stdlib only, all inputs passed as arguments. Set ROUND_SPLIT_MODE=llm in the
app to bypass this and use the API instead.
"""
